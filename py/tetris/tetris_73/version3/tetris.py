...
pd.set_mode((320,240),FULLSCREEN);sk=pd.get_surface();f=[[1]+[0 for x in range(8)]+[1] for x in range(19)]+[[1 for x in range(10)]];of=cdc(f);s=12;brt=Rect((100,0,s,s));b=-1;p=[];lc=[-9,0];t=0;bt=60;pg.key.set_repeat(200,100);rh=0;cr=[];crs=pg.Surface((8*s,s));crs.fill((255,0,0));crs.set_alpha(100);gv=-1;z=pg.font.Font("c.ttf",14);_=0;pg.mixer.music.load("t.ogg");pg.mixer.music.play(-1)
while 1:
 ...
 for l in p:
  ...
  for k in l:
   ...
   while c+lc[0]>8:
    lc[0]-=1
   if f[int(r+lc[1])][int(c+lc[0])] and k:
    if lc[1]==0:gv=10
    rx=1
   r+=1
  c+=1
