...
def choose_emptychar():
    global emptychar

    while True:
        print("#####")
        print("choosing emptychar (currently set to '.')")
        choice = input("just press enter for default '.', \n1 for '~', \n2 for '-', \n3 for custom.\n>> ")
        if choice == '':
            emptychar = '.'
            break
        try:
        ...
        if choice == 1:
            ...
            emptychar = input("choose your custom emptychar: ")
            break
        else:
...
def game_loop():
    ...
    global emptychar
    board_initialize(board, char=emptychar)
    display_board(board)
